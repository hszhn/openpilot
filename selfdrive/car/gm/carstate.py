...
def is_non_adaptive_cruise(car_fingerprint, cruise_enabled, acc_cruise_state, acc_cmd_active):
  ...
  return acc_cruise_state not in (2, 3)
  ...
  def update(self, pt_cp, cam_cp, loopback_cp, frogpilot_toggles):
    ...
    if self.CP.networkLocation == NetworkLocation.fwdCamera and not self.CP.flags & GMFlags.NO_CAMERA.value:
      ...
      if self.CP.pcmCruise:
        acc_status = cam_cp.vl["ASCMActiveCruiseControlStatus"]
        ret.cruiseState.nonAdaptive = is_non_adaptive_cruise(
          self.CP.carFingerprint,
          ret.cruiseState.enabled,
          acc_status["ACCCruiseState"],
          acc_status["ACCCmdActive"],
        )
